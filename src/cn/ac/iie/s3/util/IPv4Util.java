...
public class IPv4Util {
     ...
     * @return int
     */
    public static int ipV4ToInt(String ipAddr) {
        try {
    ...
    /***
     * 根据IP地址和掩码长度获取 掩码后的字符串
     * @param ipaddr
     * @param netMask
     * @return
     */
    public static String getIPV4Mask(String ipaddr, int netMask) {
         if (netMask < 0 || netMask > 32) {
             throw new IllegalArgumentException("invalid netmask with: "
                     + netMask);
         }
         
            long maskNum=1;
        	maskNum=((maskNum<<netMask)-1)<<(32-netMask);
         int ipInt = ipV4ToInt(ipaddr);
         int netIP=(int) (ipInt&maskNum);
         return intToIp(netIP);
    }
    public static String getIPV6Mask(String ipaddr, int netMask) {
        if (netMask < 0 || netMask > 128) {
           ...
           long maskNum=1;
       	maskNum=((maskNum<<netMask)-1)<<(1-netMask);
        int ipInt = ipV4ToInt(ipaddr);
        int netIP=(int) (ipInt&maskNum);
        return intToIp(netIP);
   }
    ...
    public static int[] getIPIntScope(String ipAddr, String mask) {
        ...
        try {
            ipInt = ipV4ToInt(ipAddr);
            if (null == mask || "".equals(mask)) {
                return new int[] { ipInt, ipInt };
            }
            netMaskInt = ipV4ToInt(mask);
            ipcount = ipV4ToInt("255.255.255.255") - netMaskInt;
    ...
    public static void main(String[] args) {
		System.out.println( getIPV4Mask("255.255.255.255",0));
	}
}
